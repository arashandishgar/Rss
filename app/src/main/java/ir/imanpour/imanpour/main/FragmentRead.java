...
public class FragmentRead extends android.support.v4.app.Fragment {
  ...
  public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
    ...
    @SuppressLint("Recycle") Cursor cursor = G.sqLiteDatabase.rawQuery("select * from Rss where unread=0", null);
  ...
  public void onResume() {
    super.onResume();
    Cursor cursor = G.sqLiteDatabase.rawQuery("select * from Rss where unread=0", null);
    if(cursor.getCount()==0){
      linearLayout.setVisibility(View.VISIBLE);
      return ;
    }else {
    ...
    recyclerView.setAdapter(new FeedAapter(DataBaseHelper.convertCoursorToArray(cursor)));
